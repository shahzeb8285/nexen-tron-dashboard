              ...
              .then((Fund) => {
                TronHelper.localContract
                  ...
                  .then(async (res) => {
                    ...
                    const totalRecyclesIn = User.totalRecyclesIn.toNumber();
                    ...
                    var income = {
                      directIncome: Math.round((( directIncome / divider) + Number.EPSILON) * 100) / 100,
                      rewardIncome: Math.round((( rewardIncome / divider) + Number.EPSILON) * 100) / 100,
                      levelIncome: Math.round((( levelIncome / divider) + Number.EPSILON) * 100) / 100,
                      recycleIncome: Math.round((( recycleIncome / divider) + Number.EPSILON) * 100) / 100,
                      upgradeIncome: Math.round((( upgradeIncome / divider) + Number.EPSILON) * 100) / 100,
                      levelRewardIncome: Math.round((( levelRewardIncome / divider) + Number.EPSILON) * 100) / 100,
  ...
  async fetchPlatformData() {
    ...
    const ownerWallet =
      (await TronHelper.localContract
        .ownerAmount().call()).toNumber() / divider;
    this.setState({
      entryFees: entryFees,
    ...
    const rewardWallet =
      (await TronHelper.localContract
        .rewardWallet().call()).toNumber() / divider;
    const levelRewardWallet =
      (await TronHelper.localContract
        .levelRewardWallet().call()).toNumber() / divider;
    const maxReferrals = (
      await TronHelper.localContract
        .maxReferrals().call()
    ).toNumber();
